...
首先，`this`总是返回一个 **对象**，简单说，就是返回属性或方法“当前”所在的对象。
...
var A = {
  name: '张三',
  describe: function () {
    return '姓名：'+ this.name;// 如果改为 (return '姓名：xxx';) 结果如何？
  }
};
...
function validate(obj, lowval, hival){
  console.log('obj: ',obj);
  console.log('value: ' + obj.value + ' name: ' + obj.name + ' size: ' + obj.size);
...
document.getElementById('btn').addEventListener('click',o.f);
</script>
</body>
</html>
```
...
func(); // this 指向 window，这样 this.count++ 创建了一个全局的 count 变量
counter.count // 0
count // NaN
```
