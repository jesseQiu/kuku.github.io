...
注意，在ES5之中，如果日期采用连词线（`-`）格式分隔，且具有前导0，JavaScript会认为这是一个ISO格式的日期字符串，导致返回的时间是以 UTC (Universal Time Coordinate 协调世界时)时区计算的。
...
## 参考链接
- [日期和时间格式](https://msdn.microsoft.com/zh-cn/library/ms141005.aspx)
