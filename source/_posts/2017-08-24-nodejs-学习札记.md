...
## 四、`body-parser` 解析错误
### 1.错误原因
在开发一个项目中，发现对方提交的 post 请求，这边在 `body-parser` 步解析报错
```js
 !!! error handers { UnsupportedMediaTypeError: unsupported charset "UTF8"at urlencodedParser (/home/wechat/code/node_modules/body-parser/lib/types/urlencoded.js:108:12)
at Layer.handle [as handle_request] (/home/wechat/code/node_modules/express/lib/router/layer.js:95:5)
at trim_prefix (/home/wechat/code/node_modules/express/lib/router/index.js:317:13)
at /home/wechat/code/node_modules/express/lib/router/index.js:284:7
at Function.process_params (/home/wechat/code/node_modules/express/lib/router/index.js:335:12)
at next (/home/wechat/code/node_modules/express/lib/router/index.js:275:10)
at app.use (/home/wechat/code/src/app.js:55:3)
at Layer.handle [as handle_request] (/home/wechat/code/node_modules/express/lib/routelayer.js:95:5)
at trim_prefix (/home/wechat/code/node_modules/express/lib/router/index.js:317:13)
at /home/wechat/code/node_modules/express/lib/router/index.js:284:7
message: 'unsupported charset "UTF8"',
charset: 'utf8',
type: 'charset.unsupported' }
```
查看了 `body-parser` 源码发送
```js
...
```
只支持 `utf-8` 写法，而第三方那边发起的请求设置为 `application/x-www-form-urlencoded;charset=utf8` 这样就导致了上面的错误。
...
app.use((req,res,next)=>{
  ...
  req.on('end', function () {
    ...
    console.log(`end: `, req.reqData.toString())
  });
  next();
});
...
```
[参考 ](https://segmentfault.com/a/1190000003061925)
...
let filePath = path.resolve('./test.pdf/png/jpg');  
...
fs.writeFileSync(path.resolve('./base.text'), base64);  
```

## 七、通过 npm 安装 windows 整套编译环境
```bash
...
[git 工程](https://github.com/felixrieseberg/windows-build-tools)
